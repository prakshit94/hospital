...
<aside
    class="fixed inset-y-0 left-0 z-50 m-3 flex flex-col overflow-hidden rounded-[2rem] border border-white/40 bg-sidebar/80 shadow-[0_28px_80px_-34px_rgba(15,23,42,0.35)] backdrop-blur-3xl transition-all duration-300 dark:border-white/8 md:translate-x-0"
    :class="{
...
>
    <div class="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.42),transparent_34%)] dark:bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.07),transparent_26%)]"></div>

    <div class="relative flex h-20 items-center border-b border-sidebar-border/60 px-5">
        <a href="/dashboard" class="flex w-full items-center gap-3.5" :class="sidebarCollapsed ? 'justify-center' : ''">
            <div class="flex h-11 w-11 items-center justify-center rounded-[1.35rem] border border-white/20 bg-gradient-to-br from-primary via-sky-500 to-indigo-600 text-white shadow-[0_24px_50px_-24px_color-mix(in_oklab,var(--primary)_60%,transparent)]">
            ...
            <div x-show="!sidebarCollapsed" class="overflow-hidden">
                ...
                <div class="mt-1 text-[10px] font-extrabold uppercase tracking-[0.25em] text-muted-foreground">Premium Console</div>
            </div>
        </a>
    </div>

    <div class="relative flex-1 space-y-8 overflow-y-auto px-3 py-6">
        @foreach($sections as $section)
            <div class="space-y-2">
                <div class="px-3" x-show="!sidebarCollapsed">
                    <div class="text-[10px] font-extrabold uppercase tracking-[0.28em] text-muted-foreground/70">{{ $section['label'] }}</div>
                </div>
